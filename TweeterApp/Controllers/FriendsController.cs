...
namespace TweeterApp.Controllers
{
    ...
    public class FriendsController : Controller
    {
        ...
        public async Task<IActionResult> List()
        {
            var me = User.Identity!.Name!.Trim().ToLowerInvariant();
            var friends = await _db.Friends
        ...
        public async Task<IActionResult> SendRequest([FromBody] SendRequestDto dto)
        {
            ...
            var toUserName = (dto?.ToUserName ?? "").Trim().ToLowerInvariant();
            if (string.IsNullOrWhiteSpace(toUserName) || toUserName == me) return BadRequest("invalid");
                ...
                switch (existing.Status)
                {
                    ...
                    case FriendshipStatus.Pending:
                        ...
                        if (existing.RequesterUserName == toUserName && existing.AddresseeUserName == me)
                        {
                            ...
                            existing.UpdatedAt = DateTimeOffset.UtcNow;
                            await _db.SaveChangesAsync();
                            await _hub.Clients.User(toUserName).SendAsync("FriendRequestResult", me, "accepted");
                            await _hub.Clients.User(me).SendAsync("FriendListUpdated");
                            return Ok(new { status = "accepted" });
                        }
                        return Conflict("Already pending");
        ...
        public record RespondDto(string FromUserName, bool Accept);
        public record SendRequestDto(string ToUserName);
        ...
        {
            var me = User.Identity!.Name!.Trim().ToLowerInvariant();
            var from = dto.FromUserName?.Trim().ToLowerInvariant();

            var fr = await _db.Friends.FirstOrDefaultAsync(f =>
                f.RequesterUserName == from &&
            ...
            await _hub.Clients.User(from!).SendAsync("FriendRequestResult", me, dto.Accept ? "accepted" : "declined");
            await _hub.Clients.Users(me, from!).SendAsync("FriendListUpdated");

            return Ok(new { status = dto.Accept ? "accepted" : "declined" });
        }

        [HttpDelete("{otherUserName}")]
        public async Task<IActionResult> Remove([FromRoute] string otherUserName)
        {
            var me = User.Identity!.Name!.Trim().ToLowerInvariant();
            otherUserName = otherUserName?.Trim().ToLowerInvariant() ?? "";
            if (string.IsNullOrWhiteSpace(otherUserName) || otherUserName == me) return BadRequest("invalid");
            ...
            await _hub.Clients.Users(me, otherUserName).SendAsync("FriendListUpdated");
            return Ok(new { status = "removed" });
        }

        [HttpPost("cancel")]
        public async Task<IActionResult> Cancel([FromBody] string withUserName)
        {
            var me = User.Identity!.Name!?.Trim().ToLowerInvariant();
            var other = (withUserName ?? "").Trim().ToLowerInvariant();
            if (string.IsNullOrWhiteSpace(other) || other == me) return BadRequest("invalid");
            var fr = await _db.Friends.FirstOrDefaultAsync(f =>
            f.Status == FriendshipStatus.Pending &&
